...
<table width="541" border="1" align="center" cellpadding="0" cellspacing="0" class="bordes">
  <tr bgcolor="#0000FF">
    <th colspan="6" bgcolor="#CCCCCC" class="Estilo1" scope="col">Consulta de Habitantes</th>
  </tr>
  <tr bgcolor="#0000FF">
  ...
  <?php do { ?>
    <tr>
      ...
      <td><div align="center"><span class="Estilo1"><?php echo "<a href='detalles_persona.php?cedula=$row_per[cedula]'>Detalle</a>"; ?></span></div></td>
      <td><div align="center"><span class="Estilo1"><? echo "<a href='modificar_persona.php?cedula=$row_per[cedula]'>Modificar</a>";?></span></div></td>
      <td><div align="center"><span class="Estilo1"><? echo "<a onClick='return validar()' href='eliminar_persona.php?id=$row_per[id_persona]'>Eliminar</a>"; ?></span></div></td>
    </tr>
...
<table border="0" align="center">
  <tr>
    <td><?php if ($pageNum_per > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_per=%d%s", $currentPage, 0, $queryString_per); ?>"><img src="First.gif" /></a>
    <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_per > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_per=%d%s", $currentPage, max(0, $pageNum_per - 1), $queryString_per); ?>"><img src="Previous.gif" /></a>
    <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_per < $totalPages_per) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_per=%d%s", $currentPage, min($totalPages_per, $pageNum_per + 1), $queryString_per); ?>"><img src="Next.gif" /></a>
    <?php } // Show if not last page ?></td>
    <td><?php if ($pageNum_per < $totalPages_per) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_per=%d%s", $currentPage, $totalPages_per, $queryString_per); ?>"><img src="Last.gif" /></a>
    <?php } // Show if not last page ?></td>
  </tr>
</table>
</body>
